...
def get_model(
    ...
    try:
        ...
        elif model_type == ModelType.ZpGpt:
            logging.info(f"正在加载ZpGpt模型: {model_name}")
            from .ZpGpt import ZpGptClient
            access_key = os.environ.get("ZPGPT_API_SECRET", access_key)
            model = ZpGptClient(
                model_name=model_name,
                api_key=access_key,
                system_prompt=system_prompt,
